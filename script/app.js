...
let burgersBtn = document.querySelectorAll('.wrapper__list-btn'),
    ...
    loadingIn = document.querySelector('.loading__path'),
    loadMap = document.querySelector('.loading');

console.log(loadingIn);

let indexLoading = 0;
let colorArr = ['red', 'blue', 'yellow', 'white', 'lightgreen', 'purple'];
...
function loadingFunc() {
    if (indexLoading < 100) {
        indexLoading++
        setTimeout(() => loadingFunc(), 28);
        loadingIn.innerHTML = `loading ${indexLoading}`
        loadingIn.style.color = colorArr[mathRandom(0, 5)]
    } else {
